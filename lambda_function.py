...
def _display_dt(s: str) -> str:
    if not s:
        return ""
    s = s.strip()
    # ISOなら fromisoformat で整形
    try:
...
def build_notice_message(guild_id: str, notice: dict, ack_count: int, member_count: int):
    ...
    remind_at = (notice.get("remind_at") or "").strip()
    remind_line = ""
    if remind_at:
        remind_disp = _display_dt(remind_at)
        remind_line = f"⏰ {remind_disp} までに確認がない場合、未確認者にメンションが届きます。\n\n"
...
def build_notice_list_ephemeral(guild_id: str, event_id: str, notices: list[dict]):
    ...
    if visible:
        lines.append("\n**表示中**")
        for n in visible[:5]:
            components.append(add_notice_row(guild_id, n))
...
def add_notice_row(guild_id: str, n: dict):
    nid = n["notice_id"]
    ...
    # ③ close（OPENだけ）
    if (not is_hidden) and status == "OPEN":
        row["components"].append({
            "type": 2,
            "style": 2,
            "label": "close",
...
def query_notices_by_event(guild_id: str, event_id: str, include_hidden: bool = True):
    ...
    resp = notices_table.scan(
        FilterExpression=Attr("guild_id").eq(guild_id) & Attr("event_id").eq(event_id)
    )
    items = resp.get("Items") or []
...
def lambda_handler(event, context):
    ...
    if itype == 5:
        ...
        if remind_at_dt:
            remind_iso = remind_at_dt.isoformat()
            ...
            notice_item["remind_at"] = remind_iso
        ...
        sent = discord_send_message_bot(notice_channel_id, msg)
        message_id = sent.get("id")
        print("NOTICE_POSTED message_id =", message_id)

        notice_item["notice_message_id"] = message_id
        notice_item["notice_channel_id"] = notice_channel_id
        ...
        print("NOTICE_SAVED message_id -> DDB", notice_id)
    ...
    # ===== Notice: open -> modal =====
        ...
        if k in ("notice_close", "notice_hide", "notice_show"):
            ...
            if k == "notice_close":
                ...
                # 最新を取り直す(直前にupdateされている場合がある)
                notice_latest = get_notice_item(guild_id, notice_id) or notice
                ch = notice_latest.get("notice_channel_id") or notice_latest.get("channel_id")
                mid = notice_latest.get("notice_message_id") or notice_latest.get("message_id")

                print("CLOSE_EDIT_TARGET:", {"notice_id":notice_id, "ch":ch, "mid":mid})

                if not ch or not mid:
                    print("EDIT_SKIP_MISSING_IDS_KEYS:", list((notice_latest or {}).keys()))
                    return _resp({"type": 4, "data": {"flags": 64, "content": "❌ メッセージIDが保存されていません。連絡を作り直すか、ログを確認してください。"}}, 200)

                discord_edit_message_bot(ch, mid, new_msg)
        ...
        if custom_id.startswith("join_event:"):
            ...
            try:
                ...
                cnt = count_event_members(guild_id, event_id)
                print("DEBUG MEMBER_COUNT_NOW =", cnt)
